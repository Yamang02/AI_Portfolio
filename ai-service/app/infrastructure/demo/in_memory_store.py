...
class InMemoryVectorStore:
    ...
    def __init__(self):
        ...
        self.chunks: List[Document] = []  # 청크도 Document 객체로 저장
    ...
    def add_documents(
        ...
        if len(chunks) != len(embeddings_array):
            raise ValueError(f"청크 개수({len(chunks)})와 임베딩 개수({len(embeddings_array)})가 일치하지 않습니다")
        ...
        if self.embeddings is None:
            ...
            if embeddings_array.shape[1] != self.embedding_dim:
                raise ValueError(f"임베딩 차원 불일치: 기존({self.embedding_dim}) vs 새로운({embeddings_array.shape[1]})")
    ...
    def get_store_statistics(self) -> Dict[str, Any]:
        """저장소 통계 정보 반환"""
        total_content_length = sum(len(chunk.page_content) for chunk in self.chunks)
    ...
    def _estimate_memory_usage(self) -> float:
        ...
        text_memory = sum(len(chunk.page_content.encode('utf-8')) for chunk in self.chunks)
        text_memory += sum(len(doc.page_content.encode('utf-8')) for doc in self.documents)
