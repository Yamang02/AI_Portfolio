...
class MetricsCollector:
    ...
    async def _update_api_metrics(self):
        ...
        try:
            ...
            if response_times:
                response_times.sort()
                self.api_metrics.avg_response_time = sum(
                    response_times) / len(response_times)
                self.api_metrics.p95_response_time = response_times[int(
                    len(response_times) * 0.95)]
                self.api_metrics.p99_response_time = response_times[int(
                    len(response_times) * 0.99)]
    ...
    async def _update_search_metrics(self):
        ...
        try:
            ...
            if search_times:
                self.search_metrics.avg_search_time = sum(
                    search_times) / len(search_times)

            if results_counts:
                self.search_metrics.avg_results_count = sum(
                    results_counts) / len(results_counts)
    ...
    async def get_metrics_history(
            self, hours: int = 24) -> List[Dict[str, Any]]:
        """메트릭 히스토리 반환"""
        try:
            history_key = f"{self.metrics_key_prefix}history:{datetime.now().strftime('%Y-%m-%d')}"
