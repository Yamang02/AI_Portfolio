    ...
    def create_tab(self) -> gr.Tab:
        ...
        with gr.Tab("📄 DocumentLoad", id=0) as tab:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    source_input = gr.Textbox(
                        label="출처",
                        placeholder="문서 출처를 입력하세요 (예: AI Portfolio, GitHub README 등)",
                        value="사용자 입력"
                    )
            ...
            load_sample_btn.click(
                fn=self.gradio_adapter.handle_load_sample_data,
                outputs=[sample_status, preview_output, doc_select]
            )
            add_btn.click(
                fn=self.gradio_adapter.handle_add_document,
                inputs=[doc_input, source_input],
                outputs=[add_output, preview_output, doc_select]
            )
            refresh_btn.click(
                fn=self.gradio_adapter.handle_refresh_document_list,
                outputs=doc_select
            )
            view_content_btn.click(
                fn=self.gradio_adapter.handle_get_document_content,
                inputs=[doc_select],
