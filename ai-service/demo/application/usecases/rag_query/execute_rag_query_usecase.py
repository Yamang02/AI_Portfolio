...
class ExecuteRAGQueryUseCase:
    ...
    @handle_usecase_errors(
        default_error_message="RAG Query 실행 중 오류가 발생했습니다.",
        log_error=True
    )
    @validate_required_fields(
    ...
    def execute(
        ...
        if not search_results:
            return ResponseFormatter.success(
                ...
                message="🤖 RAG Query가 실행되었지만 관련 정보를 찾을 수 없습니다"
            )
        ...
        return ResponseFormatter.success(
            ...
            message="🤖 RAG Query가 성공적으로 실행되었습니다"
        )
...
---"""
    ...
    @handle_usecase_errors(
        default_error_message="샘플 쿼리 생성 중 오류가 발생했습니다.",
        log_error=True
    )
    @log_usecase_execution("ExecuteRAGQueryUseCase")
