...
class GradioCommonComponents:
    ...
    @staticmethod
    def create_document_detail_card(
        document_id: str,
        title: str,
        source: str,
        content: str,
        ...
        if tags and len(tags) > 0:
            ...
            <div style="margin-top: 12px;">
                <div style="font-weight: 600; color: #666; margin-bottom: 6px; font-size: 12px;">🏷️ 태그</div>
                <div style="display: flex; flex-wrap: wrap; gap: 4px;">
                    {''.join([f'<span style="background: #e3f2fd; color: #1976d2; padding: 2px 8px; border-radius: 12px; font-size: 11px;">{tag}</span>' for tag in tags])}
                </div>
            </div>
            """
        ...
        if description and description.strip():
            description_html = f"""
            <div style="margin-top: 12px;">
                <div style="font-weight: 600; color: #666; margin-bottom: 6px; font-size: 12px;">📝 설명</div>
        ...
        if created_at or updated_at:
            ...
            <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #f0f0f0;">
                <div style="display: flex; justify-content: space-between; font-size: 11px; color: #888;">
                    {f'<div>📅 생성: {created_at}</div>' if created_at else ''}
                    {f'<div>🔄 수정: {updated_at}</div>' if updated_at else ''}
                </div>
            </div>
            """
        ...
        ">
            <div style="display: flex; align-items: center; margin-bottom: 16px;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="color: #666; font-size: 13px; margin-top: 4px;">📁 {source}</div>
                </div>
                <div style="text-align: right;">
                    <div style="background: #f8f9fa; padding: 4px 8px; border-radius: 6px; font-size: 11px; color: #666;">
                        {document_type}
                    </div>
    ...
    @staticmethod
    def create_choices_from_list(items: List[Dict[str, Any]], 
                                title_key: str = "title", 
                                id_key: str = "id",
                                id_prefix: str = "") -> List[str]:
        """범용 선택 옵션 생성기"""
        choices = []
        
        for item in items:
            title = item.get(title_key, "제목 없음")
            item_id = item.get(id_key, "")
            id_display = f"{id_prefix}{item_id[:8]}..." if item_id else ""
            choices.append(f"{title} ({id_display})" if id_display else title)
        
        return choices
    
    @staticmethod
    def create_document_choices(result: Dict[str, Any]) -> List[str]:
        """문서 선택 옵션 생성 (범용 메서드 사용)"""
        # UseCase에서 반환된 순수 데이터 처리
        documents = result.get("documents", [])
        choices = []
        ...
        for doc in documents:
            ...
            # document_id가 없거나 빈 문자열인 경우 경고 로그 출력
            if not doc_id:
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(f"⚠️ document_id가 누락된 문서 발견: {title}")
                logger.warning(f"문서 데이터: {doc}")
                continue  # document_id가 없는 문서는 선택 목록에서 제외
            
            # 실제 document_id를 포함한 선택 옵션 생성
            choices.append(f"{title}|{doc_id}")
        
        return choices
    
    @staticmethod
    def create_chunk_choices(result: Dict[str, Any]) -> List[str]:
        """청크 선택 옵션 생성 (범용 메서드 사용)"""
        if not result.get("success", False):
            return []
        
        chunks = result.get("data", [])
        return GradioCommonComponents.create_choices_from_list(
            chunks, 
            title_key="chunk_preview", 
            id_key="chunk_id",
            id_prefix="청크 "
        )
