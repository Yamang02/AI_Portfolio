...
class PromptManager:
    ...
    def get_task_template(self, template_name: str, template_key: str) -> Optional[Dict[str, Any]]:
        """작업별 템플릿 반환"""
        if not self._loaded:
            self.load_prompts()
        ...
        for template_name, templates in self.task_templates.items():
            for template_key, template in templates.items():
                if 'system' not in template:
                    errors["task_templates"].append(f"시스템 프롬프트 키 누락: {template_name}.{template_key}")
                if 'human_template' not in template:
                    errors["task_templates"].append(f"휴먼 템플릿 누락: {template_name}.{template_key}")
