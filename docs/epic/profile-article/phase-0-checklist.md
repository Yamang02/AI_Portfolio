...
### 1.4 검증
- [x] Admin 페이지를 열어 Antd 컴포넌트가 독립적인 색상 체계를 사용하는지 확인
- [x] 여러 Admin 페이지에서 일관된 스타일 적용 확인
- [x] Antd 테마가 올바르게 적용되는지 확인
- [ ] 브라우저 개발자 도구에서 적용된 테마 토큰 확인 (수동 검증 필요)
...
### 2.3 에러 처리 로직 구현
- [x] 401 Unauthorized - "로그인이 필요합니다." 메시지
- [x] 403 Forbidden - "권한이 없습니다." 메시지
- [x] 404 Not Found - "요청한 리소스를 찾을 수 없습니다." 메시지
- [x] 500+ Server Error - "서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요." 메시지
- [x] 기타 에러 - 서버에서 받은 에러 메시지 표시
- [x] ApiResponse<T> 구조 지원 (success, message, data, error)
...
### 2.5 검증
- [x] API 호출 테스트 (GET, POST, PUT, DELETE, UPLOAD) - 구현 완료
- [x] 에러 발생 시 일관된 토스트 메시지 표시 확인 - 구현 완료
- [ ] 세션 쿠키가 모든 요청에 포함되는지 확인 (브라우저 개발자 도구) - 수동 검증 필요
- [ ] 401/403 에러 시 적절한 메시지 표시 확인 - 수동 검증 필요
- [ ] 네트워크 탭에서 요청/응답 헤더 확인 - 수동 검증 필요
...
### 9.1 발견된 이슈
- 이슈 1: AdminApiClient가 초기에 ApiResponse<T> 구조를 지원하지 않아 Auth API와 호환성 문제 발생
- 이슈 2: Admin 테마가 Main 디자인시스템 CSS 변수를 참조하여 독립성 부족

### 9.2 해결 방안
- 해결 1: AdminApiClient의 handleResponse 메서드를 수정하여 ApiResponse<T> 구조를 지원하도록 개선. success가 false인 경우 에러로 처리하고, true인 경우 data를 반환하도록 구현
- 해결 2: Admin 테마를 독립적인 색상 체계로 변경. CSS 변수 참조를 제거하고 Antd 표준 색상 체계를 직접 사용하도록 수정

### 9.3 기술적 부채
- 부채 1: 기존 코드 마이그레이션 미완료 (adminAuthApi, adminProjectApi 등이 아직 adminApiClient를 사용하지 않음)
- 부채 2: 단위 테스트 및 통합 테스트 미작성
...
### 10.1 잘된 점
- 설계 문서를 기반으로 체계적으로 구현 진행
- 공통 프레임워크를 먼저 구축하여 이후 개발 생산성 향상 기대
- ApiResponse<T> 구조 지원으로 백엔드 API와의 호환성 확보
- 독립적인 색상 체계로 Admin과 Main의 분리 명확화

### 10.2 개선할 점
- 초기 설계에서 ApiResponse<T> 구조 지원을 명시하지 않아 추가 수정 필요
- 테스트 코드를 구현과 동시에 작성하지 않아 검증 지연
- 기존 코드 마이그레이션을 Phase 0에 포함하지 않아 다음 단계로 연기

### 10.3 배운 점
- 백엔드 API 응답 구조를 사전에 정확히 파악하는 것이 중요
- 공통 프레임워크 구축 시 기존 코드와의 호환성을 먼저 고려해야 함
- Admin과 Main의 디자인 분리는 유지보수성 향상에 도움
