# Epic E04: cache-boundary-and-ui-primitives-refactor
...
- E02/E03에서 드러난 구조적 재발 요인(캐시 포맷 결합, 계약 검증 부재)을 기준 문서와 코드에 반영해 운영 절차를 고정한다.

## 배경 / 맥락

### 현재 상태

E02와 E03 구현 이후, null-safety/정렬 유틸/기능 제거는 완료되었지만 다음 구조 문제가 추가로 확인됐다.
...
- E03 범위(아티클 통계 null-safety)와 구조 리팩터링 범위를 분리하지 않으면 완료 기준이 흐려진다.

## 아키텍처 선택 (요약)

| 축 | 선택 | 근거 |
|----|------|------|
| 백엔드 레이어 | 단일 Spring Boot 모듈, 직렬화·Redis 설정은 `infrastructure/config` | E02와 동일하게 인프라 관심사를 설정 계층에 둔다. Domain/Application 시그니처는 바꾸지 않고 **빈 경계(ObjectMapper)** 만 분리한다. |
...
- E03은 기능 회귀 수정 에픽으로 유지하고, 구조 리팩터링은 E04로 분리한다.
