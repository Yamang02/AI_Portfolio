package com.aiportfolio.backend.application.service.validation;
...
public class InputValidationService {
    ...
    public static class ValidationResult {
        ...
        private final ChatResponseType responseType;
        
        public ValidationResult(boolean valid, String reason, ChatResponseType responseType) {
            this.valid = valid;
            this.reason = reason;
    ...
    public ValidationResult validateInput(String input) {
        if (input == null || input.trim().isEmpty()) {
            return new ValidationResult(false, "질문을 입력해주세요.", ChatResponseType.INVALID_INPUT);
        }
        ...
        if (trimmedInput.length() < 2) {
            return new ValidationResult(false, "질문을 2자 이상 입력해주세요.", ChatResponseType.INVALID_INPUT);
        }
        
        if (trimmedInput.length() > 500) {
            return new ValidationResult(false, "질문은 500자 이하로 입력해주세요.", ChatResponseType.INVALID_INPUT);
        }
        
        // 2. 스팸 패턴 검증
        if (isSpamPattern(trimmedInput)) {
            return new ValidationResult(false, "적절한 질문을 입력해주세요. 포트폴리오나 프로젝트에 대한 질문을 해주시면 도움을 드릴 수 있습니다.", ChatResponseType.SPAM_DETECTED);
        }
        
        // 3. 의미 없는 반복 문자 검증
        if (isMeaninglessRepetition(trimmedInput)) {
            return new ValidationResult(false, "의미 있는 질문을 입력해주세요.", ChatResponseType.SPAM_DETECTED);
        }
        
        // 4. 한글/영문/숫자/특수문자 비율 검증
        if (!hasReasonableCharacterRatio(trimmedInput)) {
            return new ValidationResult(false, "적절한 질문을 입력해주세요.", ChatResponseType.SPAM_DETECTED);
        }
        
        return new ValidationResult(true, null, ChatResponseType.SUCCESS);
    }
